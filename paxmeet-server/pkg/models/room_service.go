...
func (r *RoomService) CreateRoom(roomId string, emptyTimeout *uint32, maxParticipants *uint32, creator string, metadata string) (*livekit.Room, error) {
...
func (r *RoomService) UnmarshalParticipantMetadata(metadata string) (*plugnmeet.UserMetadata, error) {
	...
	if metadata == "" {
		...
		metadataID := uuidWithHyphen.String()
		metadata = "{\"is_admin\":true,\"is_presenter\":false,\"raised_hand\":false,\"wait_for_approval\":false,\"metadata_id\":\"agent-" + metadataID + "\"}"
	}
